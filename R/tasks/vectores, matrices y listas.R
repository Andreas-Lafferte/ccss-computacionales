...
vec_num <- seq(1,10, length.out = 12)
...
vec_comb <- c(vec_num[1:4], vec_int[6:10])
round(vec_comb, digits = 2)
...
valor_5.3 <- as.numeric(mat_3[5,c(3)])
# 7. Usando "cbind()" combinar "vec_num", "vec_int", "vec_cha", y "vec_fac" en "mat_4".
mat_4 <- cbind(vec_num[1:10], vec_int, vec_cha, vec_fac)
...
mat_final <- rbind(mat_3, mat_4)
mat_final
# 11. Asignar filas y columnas con comando paste() y rep()
rownames(mat_final) <- paste(rep(c(1:10), times = 2))
colnames(mat_final) <- paste(rep(c("Numerico", "Integral", "Character", "Factor"), each = 1))
